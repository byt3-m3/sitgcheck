...
class device:
    ...
    def send_command(self, command):
     ...
     if self.status == True:
      ...
      time.sleep(3)
      ssh_output = remote_conn.recv(10000)
    ...
    def pull_interfaces(self):
            try:
                ...
                results = str(pattern.findall(ssh_output)).replace("\\r", "").splitlines()
...
# print "Command output stored in variable \n 'ssh_output'"
...
def check_net0441(device):
 ...
 aaa_pattern = re.compile("^aaa.authentication.login.*local$", re.I|re.M)
 device.connect()

 device.send_command("show run | inc ^username")
 user_results =  ssh_output
 user_final =  str(user_pattern.findall(user_results))
 print user_results
 


 print user_final.count("username")
 if user_final.count("username") == 1:
  print user_results
